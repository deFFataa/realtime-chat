...
const PersonalInformation = ({ user }: Props) => {
    ...
    const [month, setMonth] = useState(new Date(data.date_of_birth).getMonth());
    const [year, setYear] = useState(new Date(data.date_of_birth).getFullYear());

    const years = Array.from({ length: 100 }, (_, i) => new Date(data.date_of_birth).getFullYear() - i);

    const savePersonalInformation = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        post(route('admin.users.update-admin-personal-information', user.id), {
            preserveScroll: true,
            onSuccess: () => {
