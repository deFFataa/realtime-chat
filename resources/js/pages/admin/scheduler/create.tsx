...
const create = () => {
    ...
    useEffect(() => {
        // If the date changes, update the form state to reflect the selected date
        if (date) {
            setData('date_of_meeting', date.toISOString().split('T')[0]); // Store only the date part (YYYY-MM-DD)
        }
    }, [date, setData]);
                ...
                <form onSubmit={handleSubmit} className="mt-4">
                    <div className="grid gap-3">
                        <div className="grid w-full items-center gap-1.5">
                            ...
                            <Input name="title" value={data.title} onChange={(e) => setData('title', e.target.value)} id="title" placeholder="E.g. Team Meeting" />
